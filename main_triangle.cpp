...
int main3()
{
    ...
    // 你可以解绑掉这个VAO，这样后面别的VAO不会意外地修改这个VAO，不过这种情况很少发生
    // 修改其他VAO需要先调用glBindVertexArray，因此我们在没有必要时通常不会解绑VAO（或VBO）。
    // glBindVertexArray(0);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //使用从第一个VAO来的数据绘制三角形
        glBindVertexArray(VAOs[0]);     //你会看到这里我们只有一个VAO，因此不需要每次都绑定它，但还是把代码组织好。
